...
> 库内共 **21 张表**（2026-09 由 42 张收敛而来）。完整权威表结构见 [sql/schema.sql](../../sql/schema.sql)，
> 瘦身过程与逐表判定见 [06-数据库瘦身审计报告](./06-数据库瘦身审计报告.md)。
> 本表仅列核心业务表；另有 `kline_coverage` / `stock_financial` / `stock_industry` /
> `stock_valuation_daily` / `stock_news` / `research_reports` / `agent_threads` /
> `decision_cards` / `style_profiles` / `feedback_signals` 等业务表，
> 以及 LangGraph 专用的 `checkpoint*` 4 张表（HITL 线程状态）。
