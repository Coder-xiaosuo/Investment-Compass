...
from shared.datetime_ts import datetime_to_ts_ms
...
def symbol_with_sina_prefix(symbol: str) -> str:
    """将 6 位股票代码转换为新浪格式（带 sh/sz 前缀）。

    规则：
    - 60/68/90 开头 → 沪市 sh
    - 00/30 开头 → 深市 sz
    - 43/83/87/88 开头 → 北市 bj
    - 399 开头 → 深市指数 sz
    - 000 开头 → 沪市指数 sh
    """
    sym = normalize_ashare_symbol(symbol)
    ...
    if sym.startswith("399"):
        return f"sz{sym}"
    if sym.startswith(("000", "001", "002")):
        return f"sz{sym}"
    if sym.startswith(("60", "68", "90")):
        return f"sh{sym}"
    if sym.startswith(("30",)):
        return f"sz{sym}"
    if sym.startswith(("43", "83", "87", "88")):
        return f"bj{sym}"
...
def rows_to_kline_bars(rows_newest_first: list[dict[str, Any]], n: int) -> list[KlineBar]:
    from shared.ashare_limits import effective_pct_chg, prev_trading_day_close
