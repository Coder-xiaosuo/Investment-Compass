---
name: a-share-limit-up-strategy
description: A股涨跌停板的特殊规则和交易策略。使用当 Codex 分析A股市场且涉及涨跌停板时。包含：涨停板的类型和含义、跌停板的类型和含义、涨跌停附近的入场/出场策略、封板/开板信号识别。
---
...
本技能包包含【A股涨停板策略】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/A股涨停板策略.txt`
