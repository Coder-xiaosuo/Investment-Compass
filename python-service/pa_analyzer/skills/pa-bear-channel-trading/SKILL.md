---
name: pa-bear-channel-trading
description: Al Brooks 下跌通道的顺势做空交易策略。使用当 Codex 已确认下跌通道且方向为 bearish/AIS，需要生成具体入场方案时。包含：均线回撤卖进（L1/L2）、旗形突破测试卖进、突破测试失败后的顺势入场。
---
...
本技能包包含【下跌通道交易策略】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/下跌通道交易策略.txt`
