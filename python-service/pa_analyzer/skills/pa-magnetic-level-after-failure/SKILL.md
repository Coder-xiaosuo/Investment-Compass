---
name: pa-magnetic-level-after-failure
description: Al Brooks 信号失败后磁力位的完整知识。使用当 Codex 观察到交易信号失败、需要分析价格可能回撤的目标位时。包含：磁力位定义、磁力位的计算规则、磁力位在止损和目标设定中的应用。
---
...
本技能包包含【信号失败后的磁力位】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/文件22-信号失败后的磁力位.txt`
