---
name: pa-bull-channel-trading
description: Al Brooks 上涨通道的顺势做多交易策略。使用当 Codex 已确认上涨通道且方向为 bullish/AIL，需要生成具体入场方案时。包含：均线回撤买进（H1/H2）、旗形突破测试买进、突破测试失败后的顺势入场。
---
...
本技能包包含【上涨通道交易策略】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/上涨通道交易策略.txt`
