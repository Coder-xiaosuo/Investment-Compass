---
name: pa-barbed-wire-no-trade
description: Al Brooks 铁丝网形态（Barbed Wire）和无交易环境的完整知识。使用当 Codex 需要识别铁丝网形态或判断无交易环境时。包含：铁丝网形态识别标准、铁丝网中的交易限制、无交易环境的定义和处理、Wait状态的决策逻辑。
---
...
本技能包包含【铁丝网与无交易环境】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/文件21-铁丝网与无交易环境.txt`
