---
name: pa-double-top-bottom
description: Al Brooks 双重顶/双重底和微型结构的完整知识。使用当 Codex 需要识别和交易双重顶/双重底时。包含：双重顶识别标准、双重底识别标准、微型结构分析、小级别形态的交易策略。
---
...
本技能包包含【双重顶底与微型结构】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/文件28-双重顶底与微型结构.txt`
