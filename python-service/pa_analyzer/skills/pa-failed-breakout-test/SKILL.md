---
name: pa-failed-breakout-test
description: Al Brooks 突破失败（Failed Breakout）与突破测试的完整知识。使用当 Codex 观察到突破后缺乏跟随、需要评估假突破或突破测试时。包含：FBO识别标准、突破测试模式、FBO反手交易策略。
---
...
本技能包包含【突破失败与突破测试】的完整价格行为交易知识，源自 Al Brooks 价格行为体系。

源文件：`prompts/文件18-突破失败与突破测试.txt`
