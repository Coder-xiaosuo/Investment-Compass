---
source: prompts/文件20-AlwaysIn与20GB.txt
skill: pa-always-in-20gb
---
...
【A 股适配】T+1 波段 Always In

A 股 T+1 制度下，Always In 的含义从"日内多空切换"变为"日级别持仓方向"：
- AIL（Always In Long）= 日级别适合持股方向（偏多持仓）
- AIS（Always In Short）= 日级别适合持币/做空方向（偏空观望）

翻转规则（A 股修正）：
- 须至少 2 根确认 K 线（连阳/连阴）才能翻转 Always In 方向
- 单根反转棒不可触发 Always In 翻转（防止 T+1 下频繁切换）
- 涨停板处的 AIL 不可作为买入依据（无法成交）
- 跌停板处的 AIS 不可作为卖出依据（无法平仓）
- Always In 翻转后，须等待至少 1 根 K 线确认才能作为阶段二的方向参考
